    ...
    public class RunBuildRequest
    {
        ...
        internal static UnityWebRequest BuildHttpRequestMessage(string operationId, RunBuildRequest value, string baseUrl)
        {
            if("RunBuild" == operationId)
            {
                ...
                formData.Add(new MultipartFormFileSection(name, bytes, filename, "application/octet-stream"));
                // #request-debugging A, method "post"
                var urlString = baseUrl + $"/builds/v1/{appId}/run/{buildId}";
                UnityWebRequest  message = UnityWebRequest.Post(urlString, formData);
                return message;
            }
